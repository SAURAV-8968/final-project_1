...
if(isset($_POST['submit'])){
   ...
   if(!empty($Fname)){
      $update_name = $conn->prepare("UPDATE `users` SET Fname = ? WHERE id = ?");
      $update_name->execute([$Fname, $user_id]);
      $success_msg[] = 'name updated!';
   }

   if(!empty($Lname)){
      $update_name = $conn->prepare("UPDATE `users` SET Lname = ? WHERE id = ?");
      $update_name->execute([$Lname, $user_id]);
      $success_msg[] = 'name updated!';
   }
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <img src="uploaded_images/<?= $fetch_profile['image']; ?>" alt="">

      </div>

      <input type="tel" name="Fname" maxlength="50" placeholder="<?= $fetch_user['Fname']; ?>" class="box">
      <input type="tel" name="Lname" maxlength="50" placeholder="<?= $fetch_user['Lname']; ?>" class="box">
